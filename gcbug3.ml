...
module WeakDummyTable = Ephemeron.K1.Make (DummyWrapper)
...
let create_dummy =
  let dummy_table = WeakDummyTable.create 17 in
  fun num ->
    let new_dummy = wrapper_create_dummy num in
    try WeakDummyTable.find dummy_table new_dummy with
    | Not_found ->
      WeakDummyTable.add dummy_table new_dummy new_dummy;
      new_dummy
